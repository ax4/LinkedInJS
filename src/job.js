...
function search(keyword){
  var url = "https://www.linkedin.com/search/results/jobs/?keywords=" + keyword;
  //create a new instance of nightmare to use
  var nightmare = Nightmare({ show: true, webPreferences: {webSecurity:false,partition:"persist: linkedin"} });
    ...
    .wait(1000)
    .evaluate(function(){
      var list = document.querySelectorAll(".search-result__result-link")
      var href_list = []
      for (var i = 0; i < list.length; i++){
        href_list.push(list[i].href)
      }
      return href_list; 
    })
    .then(function(res){
      console.log("\n\nSearch "+ keyword + "reuslts", res); 
    })
	})
	.then(function() {
    	nightmare.end(function() {
